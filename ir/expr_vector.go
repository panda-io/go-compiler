...
func NewExprExtractElement(x, index Constant) *ExprExtractElement {
...
func (e *ExprExtractElement) Type() Type {
	// Cache type if not present.
	if e.Typ == nil {
		t, ok := e.X.Type().(*VectorType)
		if !ok {
			panic(fmt.Errorf("invalid vector type; expected *VectorType, got %T", e.X.Type()))
		}
		e.Typ = t.ElemType
	}
	return e.Typ
...
func NewExprInsertElement(x, elem, index Constant) *ExprInsertElement {
...
// Type returns the type of the constant expression.
func (e *ExprInsertElement) Type() Type {
	// Cache type if not present.
	if e.Typ == nil {
		t, ok := e.X.Type().(*VectorType)
		if !ok {
...
func NewExprShuffleVector(x, y, mask Constant) *ExprShuffleVector {
...
// Type returns the type of the constant expression.
func (e *ExprShuffleVector) Type() Type {
	// Cache type if not present.
	if e.Typ == nil {
		xType, ok := e.X.Type().(*VectorType)
		if !ok {
			panic(fmt.Errorf("invalid vector type; expected *VectorType, got %T", e.X.Type()))
		}
		maskType, ok := e.Mask.Type().(*VectorType)
		if !ok {
			panic(fmt.Errorf("invalid vector type; expected *VectorType, got %T", e.Mask.Type()))
		}
		e.Typ = NewVectorType(maskType.Len, xType.ElemType)
	}
	return e.Typ
}
